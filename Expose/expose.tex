%
% Source: https://git.fslab.de/mmklab/latex-templates/tree/master
%
...
%opening
\def\ThesisTitle{Grundlagen der Videokompression}
\def\ThesisAuthor{Justin Böhm und Matthias Greune}
\def\ThesisLocation{Darmstadt}
...
\def\ThesisSecondSupervisor{Prof. Dr. John Doe}
\def\ThesisExternalSupervisor{Dr. Who}
\def\ThesisExternalCompany{BBC}
\def\ThesisSubject{}
...
%%%% footer and header %%%%
\usepackage{scrpage2}%
%\pagestyle{scrheadings}%  % set heading with first section title on page
\clearscrheadfoot% 
\setheadwidth{text}%
\automark{section}% 
...
% Set first and second level items to bulletpoints instead of - and *
\def\labelitemii{•}
\def\labelitemiii{•}
...
%%%%%%%%%%%%%%%%%%%%% Startseite %%%%%%%%%%%%%%%%%%%%%%%%%%%
\include{titlepage}
\newgeometry{top=4cm, bottom=3cm, left=2cm, right=2cm}
...
%\include{chapter/summary-old} % will be deleted when text finished
