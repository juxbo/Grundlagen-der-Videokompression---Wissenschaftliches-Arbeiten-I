...
Die DCT ist eine Sonderform der Foriertransformation, bei der eine Funktion mittels Sinusschwingungen aproximiert wird. Dieser Sonderfall nutzt hierbei den Umstand aus, dass das betrachtete Intervall begrenzt ist. Durch eine vertikale Spiegelung dieses Intervalls lassen sich die Sinus Anteile herauskürzen, wobei am Ende lediglich Kosinus Anteile übrig bleiben. % Ähhhhh, noch einmal nachlesen...
...
% mit Cu und Cv ergänzen

Abbildung \ref{dct-good} zeigt das Resultat einer angewandten DCT auf einen schwarz-weißen 8x8 Pixelblock (siehe Anhang \ref{dct-good-luma). Der so genannte DC Wert ist der erste Wert der Matrix und gibt die mittlere Helligkeit an. Alle anderen Komponenten beschreiben die relative Abweichung zu diesem Wert und werden gemeinhin als AC Werte betitelt. (https://vsr.informatik.tu-chemnitz.de/~jan/MPEG/HTML/mpeg_tech.html)
